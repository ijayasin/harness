...
# File.join(rally_dir, "rest_json_c/client.rb")]
  ...
  def deserialize(params)
    t = Time.now.utc
    system("./#{params['client']} --deserialize --object #{params['object']} -n #{params['iterations']} -f #{File.basename(params['client'])}.serialized")
    Time.now.utc - t
  end

  def echo(params)
    t = Time.now.utc
    system("bash #{params['client']} --echo --object #{params['object']} --host localhost -n #{params['iterations']} -f #{params['input']}")
    Time.now.utc - t
  end
